...
pub(super) enum FieldAttr {
    VecSize { vec_len_field: String, unit: SizeUnit },
}
...
impl From<&syn::Field> for NetStructField {
    fn from(field: &syn::Field) -> Self {
        let name = field.ident.clone().expect("Doesn't support unnamed fields").to_string();
        let mut s = Self {
            _field: field.clone(),
            name,
...
impl NetStructField {
    ...
    fn parse_attr_vec_len(&mut self, ts: &TokenStream) {
        ...
        let expect_group_msg = format!("Expected parenthesis with arguments after \"{}\"", VEC_LEN_ATTR_PATH);
        let expect_attr_name_msg = format!("Expected identifier \"{}\"", VEC_LEN_ATTR_PATH);
        let expect_field_name_msg = format!("Expected a vector field name for \"{}\"", VEC_LEN_ATTR_PATH);
        ...
        while it.peek().is_some() {
            ...
            let mut arg_it = expect_group(
                &mut it, Delimiter::Parenthesis, 
                expect_group_msg.as_str()
            ).into_iter().peekable();
            ...
            self.net_struct_attr.push(
                FieldAttr::VecSize { vec_len_field: vec_len_field_name, unit: len_unit }
            );
        }
    }
